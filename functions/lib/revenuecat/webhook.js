...
const getWebhookSecret = () => {
    var _a;
    return ((_a = functions.config().revenuecat) === null || _a === void 0 ? void 0 : _a.webhook_secret) || process.env.REVENUECAT_WEBHOOK_SECRET || '';
};
/**
 ...
 */
function getFirebaseUserId(event) {
    ...
    if (userId.startsWith('$RCAnonymousID:')) {
        ...
        const firebaseAlias = event.aliases.find((alias) => !alias.startsWith('$RCAnonymousID:'));
 ...
 */
async function addPurchasedCredits(userId, productId, amount) {
 ...
 */
async function setSubscriptionCredits(userId, productId, amount) {
    const userRef = db.collection('users').doc(userId);
    await db.runTransaction(async (transaction) => {
        const userDoc = await transaction.get(userRef);
        if (!userDoc.exists) {
 ...
 */
async function addCredits(userId, productId, amount) {
 ...
 */
async function resetCreditsForSubscription(userId, productId, amount) {
    const userRef = db.collection('users').doc(userId);
 ...
 */
async function updateSubscriptionStatus(userId, isSubscribed, expirationMs, productId) {
    const userRef = db.collection('users').doc(userId);
    const updateData = {
        isSubscribed,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
...
async function handleInitialPurchase(event) {
    ...
    if (productId.includes('weekly') || productId.includes('yearly')) {
        await updateSubscriptionStatus(userId, true, event.expiration_at_ms, productId);
    }
}
/**
    ...
    try {
        ...
        switch (event.type) {
            ...
            case 'SUBSCRIPTION_EXTENDED':
                ...
                await updateSubscriptionStatus(getFirebaseUserId(event) || '', true, event.expiration_at_ms, event.product_id);
                break;
            case 'TRANSFER':
